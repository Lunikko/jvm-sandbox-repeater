...
import com.alibaba.jvm.sandbox.repeater.plugin.core.impl.api.NatsBroadcaster;
...
public class StandaloneSwitch {
    ...
    private StandaloneSwitch() {
        ...
        if (standaloneMode) {
            ...
            boolean useNats = Boolean.valueOf(PropertyUtil.getPropertyOrDefault(Constants.REPEAT_USE_NATS, "true"));
            boolean useKafka = Boolean.valueOf(PropertyUtil.getPropertyOrDefault(Constants.REPEAT_USE_KAFKA, "false"));
            if (useNats) {
                broadcaster = NatsBroadcaster.getInstance();
            } else if (useKafka) {
